...
export const Prescription = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('https://smhackers.onrender.com/3000/api/v1/patient/prescription', {
          params: { patientId },
        });
  ...
  return (
    <div className="bg-[#f6fffb] h-screen">
      ...
      <div className={`flex justify-center items-center p-2 bg-search bg-back-green h-32 md:h-44 relative shadow-lg `}>
        <div className=" w-full flex justify-center items-center">
          <div className="flex items-center shrink justify-center  w-1/2 h-12 md:h-16 rounded-[20px] px-4 bg-searchbar-background-green">
            ...
            <input className="bg-white/[0] w-full h-full outline-none p-4" type="text" placeholder="Search Prescription.." />
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-around gap-10 px-10 py-6 bg-[#F6FFFBC7] sm:h-32 flex-wrap mb-1 sm:flex-nowrap">
        <Button color={'bg-[#FFF4FA]'} text={'Medication'} subtext1={'Current'} />
...
const PrescriptionUploadFunction=async (doctorid,patientid,medicines)=>{
  try{
  const response = await axios.post("https://smhackers.onrender.com/3000/api/v1/doctor/prescription",
    {
      doctor:doctorid,
      patiend:patientid,
      medicines:medicines
